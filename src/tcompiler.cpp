...
struct CCallingConv {
    ...
    Type *TypeForClass(size_t size, RegisterClass clz) {
        if (win_cconv) {
            // The Microsoft x64 ABI has no SSE class: an aggregate small enough to
            // be passed directly always goes in a single integer register.
            assert(size <= 8);
    ...
    bool ValidAggregateSize(size_t sz) {
        if (win_cconv) {
            // Microsoft x64 passes an aggregate directly only when it fits exactly
            // in one register, i.e. when its size is 1, 2, 4 or 8 bytes. SysV
            // instead splits aggregates up to two eightbytes across registers.
    ...
    void AttributeFnOrCall(FnOrCall *r, Classification *info) {
        ...
        for (size_t i = 0; i < info->paramtypes.size(); i++) {
            ...
            if (v->kind == C_AGGREGATE_MEM) {
                // byval says the callee owns the temporary copy. AArch64 does not
                // work that way, and neither does Microsoft x64, which leaves the
                // copy to the caller (see Clang's WinX86_64ABIInfo::classify). The
                // two conditions are independent: one is about the architecture,
                // the other about the OS.
                if (!aarch64_cconv && !win_cconv) {
                    addByValAttr(r, argidx, v->cctype);
                } else if (aarch64_cconv) {
